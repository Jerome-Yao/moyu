...
if case .between(let previous, let next, let progress) = holidayProgress.state(
  at: date(2026, 7, 21, 12))
{
...
let maleTarget = try retirement.targetDate(
  for: .automatic(
...
let manualTarget = try retirement.targetDate(
  for: .manual(
...
let widgetPayload = WidgetTimelineGenerator(incomeEngine: income).generate(
  from: date(2026, 5, 6, 11),
  maximumItems: 4,
  horizonDays: 2
)
precondition(widgetPayload.items.map(\.phase).prefix(3) == [.beforeLunch, .atLunch, .afterLunch])
...
let encoded = try ConfigurationJSON.encode(
